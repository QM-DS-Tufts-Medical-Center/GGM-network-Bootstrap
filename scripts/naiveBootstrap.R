...
naiveBootstrap <- function(dat, sample.id = "sample.id", cluster.id = "cluster.id", boot.size=1000, cluster.sample.ratio=1, replace=T, alpha=0.05, rho=0){
  ...
  if(sum(colnames(dat) %in% c(sample.id, cluster.id))!=2){
    ...
    #### sample ids for each bootstrap sample
    resample_id <- lapply( 1:boot.size, function(i)  sample( rownames(dat), nrow(dat), replace = replace ) )
    ...
    for( i in resample_id ){
      ...
      while(max(index_list$lengths)>=1){
        ...
        boot_sample <- rbind(boot_sample, dat[which( rownames(dat) %in% index ),])
