...
fisher_pcor <- function(dat, sample.id = "sample.id", cluster.id = "cluster.id", alpha=0.05, rho=0){
  ...
  if(sum(colnames(dat) %in% c(sample.id, cluster.id))!=2){
    ...
    for(r in rho){
      ...
      if(r==0){
        p.edges <- cbind(p.edges, 2*(pnorm( -abs(vec.R)) ) )
      }else{
        p.edges <- cbind(p.edges, pnorm( (z.rho-abs(vec.R)) )+pnorm((-abs(vec.R)-z.rho)) )
      }
    }
